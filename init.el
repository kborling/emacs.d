...
(defun kdb-dashboard ()
  "Show a daily dashboard."
  (interactive)
  (let ((buf (get-buffer-create "*Dashboard*")))
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        (erase-buffer)
        (insert (propertize (format-time-string "  %A, %B %d\n")
                            'face '(:weight bold :height 1.3)))
        (insert "\n")

        ;; Today's scheduled + deadlines
        (kdb-dashboard--section "Today")
        (let ((items (kdb-dashboard--agenda-items)))
          (if items
              (dolist (item items)
                (insert "  " item "\n"))
        ...
        (kdb-dashboard--section "Due Soon")
        (let ((deadlines (kdb-dashboard--deadlines)))
          (if deadlines
              (dolist (item deadlines)
                (insert "  " item "\n"))
            (insert "  All clear\n")))
        (insert "\n")

        ;; Open follow-ups from people files
        (kdb-dashboard--section "Follow-ups")
        (let ((followups (kdb-dashboard--followups)))
          (if followups
              (dolist (item followups)
                (insert "  " item "\n"))
            (insert "  None\n")))
        (insert "\n")

        ;; Inbox count
        (kdb-dashboard--section "Inbox")
        (let ((count (kdb-dashboard--inbox-count)))
          (insert (format "  %d items to process\n" count)))
        (insert "\n")

        ;; Recent projects (clickable)
        (kdb-dashboard--section "Projects")
        (let ((projects (when (fboundp 'project-known-project-roots)
                          (seq-take (project-known-project-roots) 6))))
          (dolist (p projects)
            (let ((abbrev (abbreviate-file-name p)))
              (insert-text-button (format "  %s" abbrev)
                                  'action (lambda (_) (project-switch-project p))
                                  'follow-link t)
              (insert "\n"))))
        (insert "\n")

        ;; Quick actions
        (insert (propertize "  Keys" 'face '(:weight bold)) "\n")
        (insert "  a Agenda  c Capture  l Claude  g Git  f Find  d Refresh\n")
        (goto-char (point-min))

        ;; Local keymap for quick actions
        (let ((map (make-sparse-keymap)))
          (set-keymap-parent map special-mode-map)
          (define-key map "a" #'org-agenda)
          (define-key map "c" #'org-capture)
          (define-key map "l" #'kdb-claude)
          (define-key map "g" #'kdb-vc-transient)
          (define-key map "f" #'project-find-file)
          (define-key map "d" #'kdb-dashboard)
          (define-key map "!" #'kdb/attention)
          (use-local-map map))
        (setq buffer-read-only t)))
    (switch-to-buffer buf)))

(defun kdb-dashboard--section (title)
  "Insert a section TITLE."
  (insert (propertize (format "  %s\n" title) 'face '(:weight bold)))
  (insert "  " (make-string 38 ?─) "\n"))

(defun kdb-dashboard--agenda-items ()
  "Get today's agenda items as strings."
  (condition-case nil
      (let ((items '()))
        ...
        (with-current-buffer "*Org Agenda*"
          ...
          (while (not (eobp))
            (let ((line (buffer-substring-no-properties
              ...
              (unless (or (string-match-p "^\\(Day-agenda\\|---\\|^$\\)" line)
                          (string-blank-p (string-trim line)))
                (push (string-trim line) items)))
            (forward-line 1))
          (kill-buffer))
        (nreverse items))
    (error nil)))

(defun kdb-dashboard--deadlines ()
  "Get upcoming deadlines (next 3 days) as strings."
  (condition-case nil
      (let ((items '())
            (today (format-time-string "%Y-%m-%d"))
            (soon (format-time-string "%Y-%m-%d"
        ...
        (dolist (file (org-agenda-files))
          (when (file-exists-p file)
            (with-temp-buffer
              ...
              (goto-char (point-min))
              (while (re-search-forward "DEADLINE: <\\([0-9-]+\\)>" nil t)
                (let ((date (match-string 1)))
                  (when (and (string<= today date) (string<= date soon))
                    (save-excursion
                      (org-back-to-heading t)
                      (when (looking-at "\\*+ \\(TODO\\|FOLLOWUP\\|WAITING\\) \\(.+\\)")
                        (push (format "[%s] %s" date (match-string 2)) items)))))))))
        (sort items #'string<))
    (error nil)))

(defun kdb-dashboard--followups ()
  "Get open follow-ups from people files."
  (let ((people-dir "~/.org/people/")
        (items '()))
    (when (and (file-directory-p people-dir)
            ...
            (let ((person nil))
              (while (not (eobp))
                (cond
                 ...
                 ((looking-at "^\\*\\*\\* TODO \\(.+\\)")
                  (push (format "%s — %s" person (match-string-no-properties 1)) items)))
                (forward-line 1)))))))
    (nreverse items)))

(defun kdb-dashboard--inbox-count ()
  "Count TODO items in inbox.org."
  (let ((inbox "~/.org/inbox.org"))
